...
const ACCOUNTS_STORAGE_KEY = "dos-bol-created-accounts-v1";
const LEGACY_ACCOUNTS_STORAGE_KEY = "dos-bol-created-accounts";
...
const copy = {
  kk: {
    ...
    heroText:
      "Dos Bol — буллингтің алдын алуға, көмек сұрауға және мектеп қауымдастығында сенімді орта құруға арналған цифрлық кеңістік.",
  ...
  ru: {
    ...
    heroText:
      "Dos Bol — цифровое пространство для профилактики буллинга, безопасного обращения за помощью и создания доверительной школьной среды.",
    ...
    problemEyebrow: "Зачем нужен Dos Bol?",
...
export default function DosBolApp() {
  ...
  if (!storageReady) {
    return <div className="app-loading" role="status" aria-live="polite"><span className="brand-mark"><ShieldCheck size={25} /></span><strong>Dos Bol</strong></div>;
  }
...
function Dashboard({ account, lang, theme, section, menuOpen, onSection, onMenu, onTheme, onLang, onLogout, createdAccounts, onCreateAccount, onDeleteAccount }: { account: Account; lang: Lang; theme: Theme; section: Section; menuOpen: boolean; onSection: (section: Section) => void; onMenu: () => void; onTheme: () => void; onLang: () => void; onLogout: () => void; createdAccounts: Account[]; onCreateAccount: (account: Account & { password: string }) => Promise<string | null>; onDeleteAccount: (account: Account) => Promise<boolean> }) {
  ...
  return <div className="dashboard-shell">
    <aside className={`dashboard-sidebar ${menuOpen ? "open" : ""}`}>
      <div className="sidebar-top"><button className="brand sidebar-brand" onClick={() => onSection("home")}><span className="brand-mark"><ShieldCheck size={22} /></span><span>Dos Bol</span></button><button className="sidebar-close" onClick={onMenu}><X size={20} /></button></div>
...
function RoleWorkspace({ role, section, lang }: { role: Role; section: Section; lang: Lang }) {
  ...
  const title = titles[section] ?? ["Раздел", "Бөлім", "Рабочее пространство Dos Bol.", "Dos Bol жұмыс кеңістігі."];
...
function EnhancedChatPanel({ lang }: { lang: Lang }) {
  const [messages, setMessages] = useState([{ from: "bot", text: lang === "ru" ? "Привет! Расскажи, что тебя беспокоит. Я помогу выбрать спокойный следующий шаг." : "Сәлем! Сені не мазалайтынын айт. Мен қауіпсіз келесі қадамды таңдауға көмектесемін." }]);
  const [draft, setDraft] = useState("");
    ...
    try {
      const response = await Promise.race([
        fetch("/api/ai/chat", {
          ...
          body: JSON.stringify({
            lang,
            messages: next.map((item) => ({ role: item.from === "bot" ? "assistant" : "user", content: item.text })),
          }),
        }),
        new Promise<never>((_, reject) => window.setTimeout(() => reject(new Error("AI timeout")), 15000)),
      ]);
      const payload = await response.json() as { answer?: string };
      if (!response.ok || !payload.answer) throw new Error("AI unavailable");
      setMessages((items) => [...items, { from: "bot", text: payload.answer! }]);
    } catch {
      setMessages((items) => [...items, { from: "bot", text: lang === "ru" ? "Я рядом. Попробуй ещё раз. При срочной опасности позови взрослого или звони 112." : "Мен осындамын. Қайта көр. Шұғыл қауіпте ересекті шақыр немесе 112-ге қоңырау шал." }]);
    } finally {
      setLoading(false);
    }
  }
  return <section className="panel-page chat-page"><div className="page-title"><span>{lang === "ru" ? "Поддержка" : "Қолдау"}</span><h1>{lang === "ru" ? "Безопасный AI-чат" : "Қауіпсіз AI-чат"}</h1><p>{lang === "ru" ? "AI помогает сориентироваться, но не заменяет взрослого или специалиста." : "AI бағыт береді, бірақ ересекті не маманды алмастырмайды."}</p></div><div className="chat-layout"><div className="chat-box"><div className="chat-header"><div className="bot-avatar"><Bot size={22} /></div><div><strong>Dos Bol Assistant</strong><span><i />{loading ? (lang === "ru" ? "Думает..." : "Ойланып жатыр...") : (lang === "ru" ? "Готов помочь" : "Көмекке дайын")}</span></div><ShieldCheck size={20} /></div><div className="message-list">{messages.map((message, index) => <div className={`message ${message.from}`} key={`${index}-${message.text}`}><span>{message.text}</span></div>)}{loading && <div className="message bot"><span className="typing">•••</span></div>}</div><form className="chat-input" onSubmit={send}><input value={draft} onChange={(event) => setDraft(event.target.value)} disabled={loading} aria-label={lang === "ru" ? "Сообщение для AI" : "AI-ға хабарлама"} placeholder={lang === "ru" ? "Напиши сообщение..." : "Хабарлама жаз..."} /><button disabled={loading} aria-label={lang === "ru" ? "Отправить сообщение" : "Хабарламаны жіберу"}><Send size={18} /></button></form></div><aside className="chat-tips"><ShieldCheck size={27} /><h3>{lang === "ru" ? "Важно" : "Маңызды"}</h3><ul><li>{lang === "ru" ? "Не отправляй личные данные" : "Жеке деректерді жіберме"}</li><li>{lang === "ru" ? "Расскажи доверенному взрослому" : "Сенімді ересекке айт"}</li><li>{lang === "ru" ? "При срочной опасности — 112" : "Шұғыл қауіпте — 112"}</li></ul></aside></div></section>;
}
